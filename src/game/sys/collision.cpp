...
bool CollisionSystem_t::Collision(Entity_t& first,Entity_t& second){
    ...
    if(fD && sD && fP && sP){
        ...
        std::cout << "Heigh width y length: " << fh << " " << fw << " " << fl << std::endl;  
        std::cout << "Heigh width y length: " << sh << " " << sw << " " << sl << std::endl;  

        std::cout << "X Y Z: : " << fh << " " << fw << " " << fl << std::endl;  
        std::cout << "X Y Z: : " << sh << " " << sw << " " << sl << std::endl; 
    ...
    for(auto& inp : manager.getComponents<InputComponent_t>()){
        for(auto& col : manager.getComponents<CollisionComponent_t>()){
                         ...
                         && character->getComponent<PhysicsComponent_t>())
            {
                ...
                {
                    std::cout << "character" << character->getEntityID() << std::endl; 
                    std::cout << "objcollider" << objcollider->getEntityID() << std::endl;                 
